...
`mov/` に対応する MP4 ファイルを配置（ファイル名は `{generation_id}.mp4`）。`.mov` ファイルも読み込めます（拡張子の大文字小文字は問いません）。

`json/` は任意です。`mov/` の動画ファイルだけでも起動できます（プロンプト等のメタ情報は付きません）。
...
#### Cloudflare 用 Web ビルド

```bash
npm run build:web    # browser-zip モードで dist/ を生成
```

Cloudflare 版では Hono サーバーを使わず、Workers Static Assets で `dist/` の静的ファイルだけを配信します。ユーザーの Sora ZIP/MP4 は Cloudflare にアップロードせず、ブラウザ上で選択した ZIP を `unzipit` と Blob URL で読み込みます。
...
**データ（json / mov）はアプリに同梱しません。** 未設定のときは**アプリ内の設定画面**が開き、mov フォルダと JSON フォルダを指定します（別々の場所でも可。検出件数が表示されます）。

**必須は mov フォルダだけです。JSON は任意** — mov フォルダの動画ファイル（`.mp4` / `.mov`）だけで起動でき、その場合は次のようになります。

- ファイル名（拡張子なし）がタイトルになり、検索対象もタイトルになります
- プロンプト・アバター絞り込み・JSON 由来の解像度表示は使えません
- 再生は通常どおり動作します。サムネイル・音声/フレーム書き出しは ffmpeg があれば同様に使えます
- `{generation_id}.mp4` 以外の名前（日本語・空白入りなど）のファイルもそのまま扱えます
- 一度 JSON を設定した後に外したい場合は、設定画面の JSON フォルダの「クリア」ボタンを押します
...
- `json/` と `mov/` がプロジェクト直下（または `SORA_JSON_DIR`/`SORA_MOV_DIR`）に存在する場合は自動検出され、設定画面はスキップされます（`mov/` だけでもスキップされます）。
- サムネイルキャッシュは desktop=userData、web=`.thumbs/` に保存されます。

### Cloudflare へのデプロイ

Cloudflare 版は `browser-zip` モードの静的アプリとして、Workers Static Assets にデプロイします。R2、D1、KV、Pages Functions、認証は使いません。

Cloudflare Dashboard で Worker を GitHub 連携し、以下を設定します。
...
`wrangler.jsonc` には Worker 名と静的アセットの配信元を定義しています。
...
`assets.not_found_handling` に `single-page-application` を指定しているため、直リンクやリロード時も React アプリへフォールバックします。

ローカルで Cloudflare Workers 相当の静的配信を確認する場合:
...
| 変数            | デフォルト | 説明                        |
| --------------- | ---------- | --------------------------- |
| `SORA_JSON_DIR` | `./json`   | JSON ファイルのディレクトリ（任意） |
| `SORA_MOV_DIR`  | `./mov`    | 動画ファイル（mp4 / mov）のディレクトリ |
...
npm run build:web    # Cloudflare 用 browser-zip ビルド
