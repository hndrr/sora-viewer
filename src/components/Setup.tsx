...
const S = {
  page: {
    minHeight: '100vh', background: '#161616', color: '#ddd',
    fontFamily: 'inherit', display: 'flex', justifyContent: 'center',
  },
  ...
  card: {
    background: '#1d1d1d', border: '1px solid #2c2c2c', borderRadius: 12,
  ...
  path: {
    flex: 1, minWidth: 0, background: '#131313', border: '1px solid #333', borderRadius: 8,
    padding: '9px 12px', fontSize: 12, color: '#cfd6cf',
    whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis',
    direction: 'rtl' as const, textAlign: 'left' as const,
  },
  pathEmpty: { color: '#666', direction: 'ltr' as const },
  btn: {
    font: 'inherit', cursor: 'pointer', borderRadius: 8, border: '1px solid #3a3a3a',
    background: '#2a2a2a', color: '#ddd', padding: '9px 16px', fontSize: 13, whiteSpace: 'nowrap' as const,
  },
  ...
  launch: {
    font: 'inherit', cursor: 'pointer', borderRadius: 8, border: '1px solid #4a7a4a',
    background: '#2d4a2d', color: '#cfe9cf', padding: '11px 26px', fontSize: 13, fontWeight: 700,
  },
...
export function Setup({ onDone }: Props) {
  ...
  useEffect(() => {
    fetch('/api/config').then(r => r.json()).then(setCfg).catch(() => setCfg({
      jsonDir: null, movDir: null, jsonCount: 0, movCount: 0, configured: false,
    }))
  }, [])
  ...
  const renderPath = (dir: string | null) =>
    dir
      ? <div style={S.path} title={dir}>{dir}</div>
  ...
  const renderStat = (dir: string | null, count: number, label: string) => {
    ...
    if (count > 0) return <div style={{ ...S.stat, ...S.statOk }}>✓ {count} {label} を検出</div>
    return <div style={{ ...S.stat, ...S.statWarn }}>⚠ {label} が見つかりません。フォルダを確認してください</div>
  }
  ...
  return (
    <div style={S.page}>
      <div style={S.wrap}>
        ...
        <p style={S.lead}>
          閲覧するデータの場所を指定してください。<br />
          JSON と動画(mov)は別々の場所にあっても構いません。指定したフォルダ<strong>そのものの中身</strong>を読み込みます。
        </p>

        <div style={S.card}>
          <h2 style={S.h2}>JSON フォルダ</h2>
          <p style={S.desc}>
            Sora エクスポートの JSON があるフォルダ。フォルダ直下に <code style={S.code}>*-generations.json</code>、
            または <code style={S.code}>sora-data-files-export-1/</code> のようなサブフォルダ（中に <code style={S.code}>generations.json</code>）がある場所を選びます。
          </p>
          <div style={S.row}>
            {renderPath(cfg?.jsonDir ?? null)}
            <button style={S.btn} onClick={() => pick('json')}>フォルダを選択…</button>
          </div>
        ...
        <div style={S.card}>
          ...
          <p style={S.desc}>動画ファイル <code style={S.code}>{'{generation_id}.mp4'}</code> が入っているフォルダ。</p>
          <div style={S.row}>
            {renderPath(cfg?.movDir ?? null)}
            <button style={S.btn} onClick={() => pick('mov')}>フォルダを選択…</button>
          </div>
      ...
      {browserFor && (
        <FolderBrowser
          ...
          onSelect={async (dir) => { const w = browserFor; setBrowserFor(null); if (w) await applyDir(w, dir) }}
